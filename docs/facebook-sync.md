...
Pulls the latest posts from the official LGU Mati Facebook Page, categorizes
...
1. **Get an Editor role on the page.** Ask the LGU Mati page admin (via Meta
   Business Suite → Settings → People) to add the BetterMati account/app as an
...
| Name              | Type         | Value                                               |
| ----------------- | ------------ | --------------------------------------------------- |
| `FTP_SERVER`      | Secret       | cPanel FTP host (e.g. `ftp.bettermati.org`)       |
